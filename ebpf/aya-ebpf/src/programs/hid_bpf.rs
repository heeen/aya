...
impl HidBpfData<'_> {
    ...
    pub fn get(&self, idx: usize) -> Option<u8> {
        // SAFETY: bounds checked, ptr valid for len bytes per kernel contract
        (idx < self.len as usize).then(|| unsafe { *self.ptr.add(idx) })
    }
...
impl HidBpfContext {
    ...
    #[inline(always)]
    pub const unsafe fn new(ctx: *mut hid_bpf_ctx) -> Self {
        Self { ctx }
    }
    ...
    #[inline(always)]
    pub const fn as_ptr(&self) -> *mut hid_bpf_ctx {
        self.ctx
    }
}
