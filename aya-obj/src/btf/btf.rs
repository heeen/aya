        ...
        for ty in self.types() {
            match ty {
                ...
                BtfType::DataSec(d) => {
                    ...
                    if name == ".ksyms" || name.starts_with(".struct_ops") {
        ...
        for i in 0..types.types.len() {
            ...
            match t {
                ...
                BtfType::DataSec(d) if features.btf_datasec => {
                    ...
                    } else if name == ".ksyms" || name.starts_with(".struct_ops") {
...
impl Object {
    ...
    pub fn fixup_and_sanitize_btf(
        ...
        if let Some(obj_btf) = &mut self.btf {
            ...
            let has_struct_ops = self
                .programs
                .values()
                .any(|p| matches!(p.section, crate::ProgramSection::StructOps { .. }));
            if has_struct_ops {
...
impl BtfTypes {
    #[expect(dead_code, reason = "retained for potential future use")]
...
mod tests {
    ...
    fn test_fixup_func_linkage_global_to_static() {
        ...
        let func_id = btf.add_type(BtfType::Func(Func::new(
            name,
            proto_id,
            FuncLinkage::Global,
        )));
    ...
    fn test_to_bytes_sanitizes_struct_ops_datasec() {
        ...
        let datasec_id = btf.add_type(BtfType::DataSec(DataSec::new(
            sec_name,
            vec![DataSecEntry {
                btf_type: var_id,
                offset: 0,
    ...
    fn test_to_bytes_header_consistency() {
        ...
        let header: btf_header = unsafe { ptr::read_unaligned(raw.as_ptr().cast()) };
        let expected_len =
            header.hdr_len as usize + header.type_len as usize + header.str_len as usize;
        assert_eq!(raw.len(), expected_len, "BTF blob size must match header");
        assert_eq!(
            header.str_off, header.type_len,
            "str_off must equal type_len"
        );
    }
}
